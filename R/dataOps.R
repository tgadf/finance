...
getUniqueValues <- function(coldata, sampleSize = 100) {
  flog.debug(paste("  Getting number of unique values with",sampleSize,"/",length(coldata)))
  nUnique <- length(unique(sample(coldata, size = sampleSize, replace = T)))
  return( nUnique )
}

getCuts <- function(coldata, nBins = 20, isLog = F, sampleSize = 100) {
  flog.debug(paste("  Getting cuts for data using",nBins))
  maxBins <- min(nBins, getUniqueValues(coldata, sampleSize))
  minVal <- min(coldata)
  maxVal <- max(coldata)
  if ( isLog ) {
    if ( minVal <= 0.0 ) { minVal <- min(coldata[coldata > 0]) }
    if ( minVal <= 0.0 ) { flog.warn("Can not generate log data for this"); return( NULL) }
    minVal <- log(minVal)
    if ( maxVal <= 0.0 ) { maxVal <- max(coldata[coldata > 0]) }
    if ( maxVal <= 0.0 ) { flog.warn("Can not generate log data for this"); return( NULL) }
    maxVal <- log(maxVal)
  }
...
splitItems <- function(items, modval) {
  flog.debug(paste("    Split items by",modval,"mods."))
...
groupItems <- function(items, modval) {
  flog.debug(paste("  Group items by",modval,"mods."))
  classIDs <- sapply(seq(items), function(x) ceiling(x/modval))
...
sortAndGroupItems <- function(values, modval, decreasing = T) {
  flog.debug(paste("  Sort and group items by",modval,"mods."))
...
rmElement <- function(element, elements) {
  flog.debug(paste("  Remove element:",element))
...
retNumeric <- function(value) {
  flog.debug(paste("  Returning numeric value of",value))
...
setCharacterToNumeric <- function(df) {
  ...
  if ( sum(indx, na.rm = T) > 0 ) {
    flog.debug(paste("  Converting",sum(indx, na.rm = T),"character columns to numeric."))
