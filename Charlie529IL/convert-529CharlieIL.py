...
def createCharlie529ILCSV(basedir = '/Users/tgadfort/Documents/finance',
                   account = "Charlie529IL", debug = False):
...
def createCharlie529ILJSON(basedir = '/Users/tgadfort/Documents/finance',
                           account = "Charlie529IL", debug = False):
    createJSON(account, basedir=basedir, debug=debug)
    showSummary(account, basedir=basedir, debug=debug, showYear = False)
    

def showCharlie529ILRecords(basedir = '/Users/tgadfort/Documents/finance',
                            account = "Charlie529IL", debug = False):
    showSummary(account, basedir=basedir, debug=debug, showYear = False)
    
def run(debug = False):
    createCharlie529ILCSV(debug=debug)
    createCharlie529ILJSON(debug=debug)
...
if __name__ == "__main__":
    ...
    account="Charlie529IL"
    if args.run:
        createCharlie529ILCSV(account=account, debug=args.debug)
        createCharlie529ILJSON(account=account, debug=args.debug)
    if args.csv:
        createCharlie529ILCSV(account=account, debug=args.debug)
    if args.json:
        createCharlie529ILJSON(account=account, debug=args.debug)
    if args.show:
        showCharlie529ILRecords(account=account, debug=args.debug)