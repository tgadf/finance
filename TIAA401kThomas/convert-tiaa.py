...
def createTIAA401kThomasBalance(basedir = '/Users/tgadfort/Documents/finance',
                               account = "TIAA401kThomas", debug = False):
...
def createTIAA401kThomasCSV(basedir = '/Users/tgadfort/Documents/finance',
                    account = "TIAA401kThomas", debug = False):
...
def createTIAA401kThomasJSON(basedir = '/Users/tgadfort/Documents/finance',
                    account = "TIAA401kThomas", debug = False):
    createJSON(account, basedir=basedir, debug=debug)
    showSummary(account, basedir=basedir, debug=debug, showYear = False)

def showTIAA401kThomasRecords(basedir = '/Users/tgadfort/Documents/finance',
                     account = "TIAA401kThomas", debug = False):
    showSummary(account, basedir=basedir, debug=debug, showYear = False)
    
def run(debug = False):
    createTIAA401kThomasBalance(debug=debug)
    createTIAA401kThomasCSV(debug=debug)
    createTIAA401kThomasJSON(debug=debug)
...
if __name__ == "__main__":
    ...
    account="TIAA401kThomas"
    if args.run:
        createTIAA401kThomasCSV(account=account, debug=args.debug)
        createTIAA401kThomasJSON(account=account, debug=args.debug)
    if args.csv:
        createTIAA401kThomasCSV(account=account, debug=args.debug)
    if args.json:
        createTIAA401kThomasJSON(account=account, debug=args.debug)
    if args.show:
        showTIAA401kThomasRecords(account=account, debug=args.debug)