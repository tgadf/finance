...
def createTIAARuthCSV(basedir = '/Users/tgadfort/Documents/finance',
                    account = "TIAARuth", debug = False):
...
def createTIAARuthJSON(basedir = '/Users/tgadfort/Documents/finance',
                    account = "TIAARuth", debug = False):
    createJSON(account, basedir=basedir, debug=debug)
    showSummary(account, basedir=basedir, debug=debug, showYear = False)

def showTIAARuthRecords(basedir = '/Users/tgadfort/Documents/finance',
                     account = "TIAARuth", debug = False):
    showSummary(account, basedir=basedir, debug=debug, showYear = False)
    
def run(debug = False):
    createTIAARuthCSV(debug=debug)
    createTIAARuthJSON(debug=debug)
...
if __name__ == "__main__":
    ...
    account="TIAARuth"
    if args.run:
        createTIAARuthCSV(account=account, debug=args.debug)
        createTIAARuthJSON(account=account, debug=args.debug)
    if args.csv:
        createTIAARuthCSV(account=account, debug=args.debug)
    if args.json:
        createTIAARuthJSON(account=account, debug=args.debug)
    if args.show:
        showTIAARuthRecords(account=account, debug=args.debug)