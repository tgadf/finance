...
def createFidelityOpenDataBalance(account = "FidelityOpenData", debug = False):
...
def createFidelityOpenDataCSV(account = "FidelityOpenData", debug = False):
    basedir = getBaseLocation()
...
def createFidelityOpenDataJSON(account = "FidelityOpenData", debug = False):
    basedir = getBaseLocation()
...
def showFidelityOpenDataRecords(account = "FidelityOpenData", debug = False):
    basedir = getBaseLocation()
    showSummary(account, basedir=basedir, debug=debug, showYear = False)
...
def run(debug = False):
    createFidelityOpenDataBalance(debug=debug)
    createFidelityOpenDataCSV(debug=debug)
    createFidelityOpenDataJSON(debug=debug)
...
if __name__ == "__main__":
    ...
    account="FidelityOpenData"
    if args.run:
        createFidelityOpenDataBalance(account=account, debug=args.debug)
        createFidelityOpenDataCSV(account=account, debug=args.debug)
        createFidelityOpenDataJSON(account=account, debug=args.debug)
    if args.csv:
        createFidelityOpenDataCSV(account=account, debug=args.debug)
    if args.json:
        createFidelityOpenDataJSON(account=account, debug=args.debug)
    if args.show:
        showFidelityOpenDataRecords(account=account, debug=args.debug)